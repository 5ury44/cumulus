...
curl -fsS "http://localhost:${PORT}/health" && echo || echo "Health endpoint not responding yet."
set -e
...
Logs:
  ...
  tail -f /var/log/cumulus-worker.log    # nohup fallback

Config:
  Edit ${ENV_FILE} and then restart:
    systemctl restart cumulus-worker || (pkill -f uvicorn && bash -lc 'nohup ${VENV_DIR}/bin/python -m uvicorn worker.server:create_app --factory --host 0.0.0.0 --port ${PORT} > /var/log/cumulus-worker.log 2>&1 &')
